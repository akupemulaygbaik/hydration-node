...
impl AggregatedOracle<AssetId, Balance, BlockNumber, EmaPrice> for DummyOracle {
    ...
    fn get_entry(
        ...
        // --- TAMBAHAN UNTUK PoC EXPLOIT ---
        if ORACLE_LAG_MOCK.with(|v| *v.borrow()) {
            return Ok(AggregatedEntry {
                price: EmaPrice::default(),
                liquidity: Liquidity {
                    a: 1_000 * ONE, // Nilai EMA Lama (Lagging)
                    b: 10_000_000 * ONE, // Nilai EMA Lama (Lagging)
                },
                volume: Volume::default(),
                oracle_age: BlockNumber::default(),
                shares_issuance: Some(1_000 * ONE), // Nilai EMA Lama (Lagging)
            });
        }
        // ----------------------------------
